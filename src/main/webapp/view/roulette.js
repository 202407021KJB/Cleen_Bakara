...
console.log("roulette.js 로드됨 ✅ (v2.1 주석 추가)");
...
function drawRoulette() {
    ...
    ctx.save();
    ctx.translate(centerX, centerY);
    ctx.rotate(currentAngle); // 현재 각도만큼 룰렛 전체를 회전
    ctx.translate(-centerX, -centerY);
...
function spinRoulette() {
    ...
    fetch(`${contextPath}/roulette?${params.toString()}`)
        ...
        .then(data => {
            ...
            // [목표 각도 계산]
            const pieceCenterAngle = (winnerIndex * arcSize) + (arcSize / 2);
            const pointerAngle = 1.5 * Math.PI; // 포인터는 270도(윗 방향)에 위치
            const targetRotation = pointerAngle - pieceCenterAngle;
            ...
            function animate(timestamp) {
                ...
                if (progress < duration) {
                    ...
                    // --- 4. 애니메이션 종료 후 최종 처리 ---
                    currentAngle = totalRotation; // 정확한 위치 보정
                    drawRoulette();
                    setTimeout(() => {
                        alert(`🎉 결과: ${winner} 🎉`);
